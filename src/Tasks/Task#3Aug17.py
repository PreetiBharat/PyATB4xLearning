...
# --> == is used to comparing if two values are equal. Ex: if x == 5 checks if x is equal to 5 and return True or false.
...
# The ^ operator in python performs a XOR operation on the operands. It compares each bit of the first operand
# to the corresponding
# bit of the second operand. If both bits are different, the resulting bit is set to 1,
# otherwise it is set to 0. The ^ operator
# is commonly used in contexts like: Cryptography: For encrypting and decrypting data
